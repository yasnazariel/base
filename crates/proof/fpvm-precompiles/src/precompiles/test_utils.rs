...
/// preimages. The closure accepts the client's [`HintWriter`] and [`OracleReader`] as arguments.
...
async fn precompile_host(
    ...
    let preimage_fetcher =
        PrecompilePreimageFetcher { map: Default::default(), last_hint: Arc::clone(&last_hint) };
...
struct PrecompilePreimageFetcher {
    ...
    map: Arc<Mutex<BTreeMap<PreimageKey, Vec<u8>>>>,
    /// The previous hint received.
    last_hint: Arc<RwLock<Option<alloc::string::String>>>,
}
    ...
    async fn get_preimage(&self, key: PreimageKey) -> PreimageOracleResult<Vec<u8>> {
        ...
        if matches!(parsed_hint.ty, HintType::L1Precompile) {
            ...
            let result = execute_native_precompile(address, input, gas).map_or_else(
                |_| alloc::vec![0u8; 1],
                |raw_res| {
                    let mut res = alloc::vec::Vec::with_capacity(1 + raw_res.bytes.len());
                    res.push(0x01);
...
impl HintRouter for PrecompileHintRouter {
    async fn route_hint(&self, hint: alloc::string::String) -> PreimageOracleResult<()> {
