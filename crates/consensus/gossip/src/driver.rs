    ...
    G: ConnectionGate,
{
    ...
    pub fn publish(
        ...
        Metrics::unsafe_block_published().increment(1);
        Ok(Some(id))
    }
    ...
    pub fn dial(&mut self, enr: Enr) {
        ...
        let Some(multiaddr) = enr_to_multiaddr(&enr) else {
            ...
            Metrics::dial_peer_error("no_tcp_socket").increment(1);
            return;
        };
    ...
    pub fn dial_multiaddr(&mut self, addr: Multiaddr) {
        ...
        if self.swarm.connected_peers().any(|p| p == &peer_id) {
            ...
            Metrics::dial_peer_error("already_connected").increment(1);
            return;
        }
    ...
    fn handle_gossipsub_event(
        ...
        match event {
            libp2p::gossipsub::Event::Message {
                ...
                Metrics::gossip_events("message", message.topic.to_string()).increment(1);
            ...
            libp2p::gossipsub::Event::Subscribed { peer_id, topic } => {
                ...
                Metrics::gossip_events("subscribed", topic.to_string()).increment(1);
            }
            libp2p::gossipsub::Event::Unsubscribed { peer_id, topic } => {
                trace!(target: "gossip", peer_id = %peer_id, topic = ?topic, "Peer unsubscribed");
                Metrics::gossip_events("unsubscribed", topic.to_string()).increment(1);
            }
    ...
    pub fn handle_event(&mut self, event: SwarmEvent<Event>) -> Option<OpNetworkPayloadEnvelope> {
        match event {
            ...
            SwarmEvent::ConnectionEstablished { peer_id, .. } => {
                ...
                Metrics::gossipsub_connection("connected").increment(1);
                Metrics::swarm_peer_count().set(peer_count as f64);
