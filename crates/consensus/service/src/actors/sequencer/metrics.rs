...
pub(super) fn update_attributes_build_duration_metrics(duration: Duration) {
    Metrics::sequencer_attributes_build_duration().set(duration);
}

#[inline]
pub(super) fn update_block_build_duration_metrics(duration: Duration) {
    Metrics::sequencer_block_building_start_task_duration().set(duration);
}

#[inline]
pub(super) fn update_seal_duration_metrics(duration: Duration) {
    Metrics::sequencer_block_building_seal_task_duration().set(duration);
}

#[inline]
pub(super) fn update_total_transactions_sequenced(transaction_count: u64) {
    Metrics::sequencer_total_transactions_sequenced().increment(transaction_count);
}

#[inline]
pub(super) fn inc_seal_step_retry(step: &'static str) {
    Metrics::sequencer_seal_step_retries_total(step).increment(1);
}

#[inline]
pub(super) fn update_seal_step_duration(step: &'static str, duration: Duration) {
    Metrics::sequencer_seal_step_duration(step).set(duration);
}
...
pub(super) fn inc_start_rejected(reason: &'static str) {
    Metrics::sequencer_start_rejected_total(reason).increment(1);
}
