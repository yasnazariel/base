...
/*
    ...
    All expected testspec results were obtained from querying proof RPC on the running geth instance `geth init crates/execution/trie-base/trie/testdata/proof-genesis.json && geth --http`.
*/
static TEST_SPEC: LazyLock<Arc<ChainSpec>> = LazyLock::new(|| {
    ChainSpec {
        chain: Chain::from_id(12345),
        genesis: serde_json::from_str(include_str!(
            "../../trie-base/trie/testdata/proof-genesis.json"
        ))
