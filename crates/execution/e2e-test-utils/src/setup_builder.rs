        ...
        + 'static,
{
    ...
    /// Preserves the legacy test builder API.
    ///
    /// Storage v2 is now the canonical default in `NodeConfig::storage_settings()`,
    /// so this no longer needs to mutate the config directly.
    pub const fn with_storage_v2(self) -> Self {
        self
    }
