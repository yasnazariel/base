...
use reth_trie::{HashedPostState, StateRoot, trie_cursor::TrieCursorFactory, updates::TrieUpdates};
    ...
    C: CachedExecutionProvider<BaseTxResult<OpHaltReason, OpTxType>> + Clone,
{
        ...
        Evm: ConfigureEngineEvm<ExecutionData, Primitives = BasePrimitives>,
    {
        ...
        self.metrics
            .block_validation
            .record_state_root(trie_output.as_ref(), root_elapsed.as_secs_f64());
        self.metrics
        ...
        if state_root != block.header().state_root() {
            ...
            self.on_invalid_block(
                ...
                &block,
                &output,
                Some((trie_output.as_ref(), state_root)),
                ctx.state_mut(),
            );
    ...
    #[instrument(level = "debug", target = "engine::tree::payload_validator", skip_all)]
    fn validate_block_inner(&self, block: &SealedBlock<BaseBlock>) -> Result<(), ConsensusError> {
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = BasePrimitives>,
    {
        ...
        let (spec_id, mut executor) = {
            ...
            let executor = BaseBlockExecutor::new(
                evm,
                ctx,
                    ...
                    match task_rx.recv_timeout(POLL_INTERVAL) {
                        ...
                        Err(RecvTimeoutError::Disconnected) => {
                            ...
                            return Ok(Ok(StateRootComputeOutcome {
                                state_root,
                                trie_updates: trie_updates.into(),
                            }));
                        }
                        Err(RecvTimeoutError::Timeout) => {}
                    }
...
where
    Evm: ConfigureEvm,
{
    fn wait_for_caches(&self) -> reth_engine_tree::tree::CacheWaitDurations {
